...
#!/usr/bin/env bash
# =============================================================================
# Rocky Linux KISA 보안조치 자동화 스크립트
...
#
# 사용법:
#   sudo ./rocky_kisa_hardening.sh [옵션]
#
# 옵션:
...
detect_os() {
    ...
    case "$OS_ID" in
        rocky|rhel|almalinux|centos)
            ...
            PAM_PASSWD_FILE="/etc/pam.d/password-auth"
            FAILLOCK_CONF="/etc/security/faillock.conf"
            SYSLOG_CONF="/etc/rsyslog.conf"
            NTP_SVC="chronyd"
            ;;
        *)
            echo -e "${RED}ERROR: 지원하지 않는 OS: ${OS_ID} — Rocky Linux 9.x 필요${NC}"
            exit 1
            ;;
    esac
}
...
pkg_install() {
    local pkg="$1"
    do_fix "[dnf] ${pkg} 설치" "dnf install -y '$pkg'"
}
...
_u02_check() {
    ...
    if [[ -f "$pq" ]]; then
        ...
        _fail "[${pq}] 파일 없음 (libpwquality 미설치)"; ok=false
    fi
...
_u02_fix() {
    ...
    rpm -q libpwquality &>/dev/null || pkg_install libpwquality
    backup_file "$pq"
...
_u03_fix() {
    ...
    _warn "authselect 활성화 전 백업 확인: ${BACKUP_DIR}/etc/pam.d/"
...
_u04_fix() {
    local s="/etc/shadow"
    [[ -f "$s" ]] && {
        do_fix "[${s}] chown root:root, chmod 000" "chown root:root '$s' && chmod 000 '$s'"
    }
...
# ── U-09: 계정 없는 GID 금지 [MANUAL] ────────────────────────────────────────
_u09_check() {
    local ok=true orphan=false
    local system_groups="kmem dialout fax voice floppy tape audio operator src utmp video sasl staff sgx kvm render crontab tty disk lp adm sys shadow cdrom plugdev netdev bluetooth scanner input man lock utempter games ssh_keys stapusr stapsys stapdev systemd-journal docker wheel"
...
_u13_check() {
    ...
    if echo "$acceptable" | grep -qw "${algo:-}"; then
        ...
        _fail "[${lf}] ENCRYPT_METHOD=${algo:-없음} (SHA512 필요)"; ok=false
    fi
    local bad_hash
...
_u17_check() {
    ...
    for target in /etc/rc.d/rc.local /etc/init.d; do
        [[ ! -e "$target" ]] && continue
...
_u18_fix() {
    do_fix "[/etc/shadow] chown root:root, chmod 000" "chown root:root /etc/shadow && chmod 000 /etc/shadow"
}
...
_u34_check() {
    [[ "$(systemctl is-active finger 2>/dev/null || echo inactive)" != "active" ]] \
...
_u35_check() {
    ...
    if systemctl is-active vsftpd &>/dev/null; then
        local anon; anon=$(grep -E "^anonymous_enable" /etc/vsftpd/vsftpd.conf 2>/dev/null | awk -F= '{print $2}' | tr -d ' ' | head -1)
        [[ "${anon,,}" == "no" ]] \
...
_u39_fix() {
    for svc in nfs-server nfs-mountd; do
        [[ "$(systemctl is-active "$svc" 2>/dev/null || echo inactive)" == "active" ]] && {
            # NFS 마운트 여부 확인
            if grep -q nfs /etc/fstab 2>/dev/null || mount | grep -q nfs; then
                _warn "[${svc}] NFS 마운트 사용 중 — 중단 전 마운트 해제 확인 필요"
            fi
            do_fix "[${svc}] stop+disable" "systemctl stop '$svc'; systemctl disable '$svc' 2>/dev/null || true"
        }
    done
}
...
_u42_fix() {
    for svc in rpcbind rpcbind.socket; do
        [[ "$(systemctl is-active "$svc" 2>/dev/null || echo inactive)" == "active" ]] && {
            if grep -q nfs /etc/fstab 2>/dev/null || mount | grep -q nfs; then
                _warn "[rpcbind] NFS 마운트 사용 중 — rpcbind 중단 전 확인 필요"
            fi
            do_fix "[${svc}] stop+disable" "systemctl stop '$svc' 2>/dev/null; systemctl disable '$svc' 2>/dev/null || true"
        }
    done
...
_u49_check() {
    if ! systemctl is-active named &>/dev/null 2>&1; then
        _na "[named] DNS 서비스 미사용 — 해당없음"
        return 0
    fi
    local ver; ver=$(named -v 2>/dev/null | head -1)
...
*******************************************************************************'
    ...
    $DRY_RUN || do_fix "sshd reload" "systemctl reload sshd 2>/dev/null"
}
...
_u63_check() {
    ...
    [[ -z "$risky" ]] \
        ...
        || { _warn "[/etc/sudoers] NOPASSWD 설정 발견:"; echo "$risky" | while read -r l; do _warn "  ${l}"; done; ok=false; }
    $ok && return 0 || return 1
}
...
_u66_check() {
    ...
    for lf in /var/log/messages /var/log/secure; do
        [[ -f "$lf" ]] && _pass "[${lf}] 파일 존재" || { _fail "[${lf}] 파일 없음"; ok=false; }
    done
...
_u66_fix() {
    ...
    local logconf="/etc/rsyslog.conf"
    if ! grep -qE "authpriv\.\*|auth,authpriv\.\*" "$logconf" 2>/dev/null; then
        backup_file "$logconf"
        do_fix "[${logconf}] 로그 정책 추가" "cat >> '$logconf' << 'EOF'
...
_u67_check() {
    ...
    local logfiles=(/var/log/messages /var/log/secure /var/log/cron /var/log/wtmp /var/log/btmp)
...
_u67_fix() {
    ...
    for lf in "${logfiles[@]}"; do
        ...
        # 이미 기준(≤640) 충족하면 skip
        [[ "$lp" -le 640 ]] || do_fix "[${lf}] chown root:root, chmod 640" "chown root:root '$lf' && chmod 640 '$lf'"
    done
}
...
parse_args() {
    while [[ $# -gt 0 ]]; do
        case "$1" in
            ...
            --help|-h)
                cat << 'EOF'
사용법: rocky_kisa_hardening.sh [옵션]
...
║  Rocky Linux 보안조치 자동화 스크립트 v1.0.0                  ║
...
main() {
    ...
    should_run "U-28" "2" && run_manual "U-28" "접속 IP 및 포트 제한"            "상" _u28_check \
        "firewall-cmd --add-rich-rule 으로 IP/포트 제한 설정"
    ...
    should_run "U-45" "3" && run_manual "U-45" "메일 서비스 버전 점검"           "상" _u45_check \
        "dnf update sendmail postfix — 최신 버전 적용"
